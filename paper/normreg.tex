...
Next, we consider what this one-dimensional solution looks like under various forms of normalization.

\subsubsection{Standardization}

Now, under the assumption that \(y_i = \sum_i x_{ij} \beta_j + \varepsilon_i\) with \(\E \varepsilon_i = 0\), we have
% FIXME: This is not corrent.
\[
...
\[
  \E \hat\beta = \frac{\softthreshold{\left( n\beta q(1-q) ; \alpha\lambda\right)}}{\sqrt{q(1-q)} \left(n + (1-\alpha)\lambda\right)}
\]
