...
\[
  \begin{aligned}
    \var Z & = \var\left(\tilde{\vec{x}}_j ^\T \vec{\varepsilon}\right) + \left(\frac{1}{n}\tilde{\vec{x}}_j^\T \ones\right)^2 n \var(\varepsilon) \\
           & = \sigma_\varepsilon^2 \lVert \tilde{\vec{x}}_j\rVert_2^2 +\frac{1}{n}(\tilde{\vec{x}}_j^\T \ones)^2 \sigma_\varepsilon^2.
  \end{aligned}
\]
...
\]
Let \(\risk(\lambda, \mu) = \E \big(\st_\lambda(Z) - \mu\big)^2\) for brevity. And now
\[
...
\]
Next, assume that \(\mu \geq 0\). Now we compute an upper bound for the mean-squared error.
\begin{equation}
  \label{eq:mse-bound1}
  \begin{aligned}
     & \lim_{\mu \rightarrow \infty} \risk(\lambda, \mu)                                                                                                                                                                                                                                                                                                                \\
     & =  \lim_{\mu \rightarrow \infty}\left(\sigma^2 \int_\frac{\lambda - \mu}{\sigma}^\infty u^2 \phi(u)\du u - 2\lambda \int_\frac{\lambda - \mu}{\sigma}^\frac{-\lambda - \mu}{\sigma} u \phi(u)\du u + \lambda^2 \int_\frac{\lambda-\mu}{\sigma}^\infty \phi(u)\du u + \mu^2 \int_\frac{-\lambda - \mu}{\sigma}^\frac{\lambda - \mu}{\sigma} \phi(u) \du u \right) \\
     & = \sigma^2 + \lambda^2.
  \end{aligned}
\end{equation}
where \(\phi(u)\) is the probability density function of the standard normal distribution. So for a given \(\lambda\) we have \(r(\lambda, \mu) \leq \sigma^2 + \mu^2\).
...
Returning to the problem of computing the mean squared error across multiple features, we have
\[
  \E \big(\hat\beta - \beta) = \sum_{j = 1}^p \risk(\lambda, \mu) \leq p \risk(\lambda, 0) + \sum_{j=1}^p \min(\sigma^2 + \lambda^2, \mu^2)
\]
