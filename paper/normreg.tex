...
\begin{align}
  \hat\beta_j & = \frac{\st_{\alpha\lambda}\left(\tilde{\vec{x}}_j^\T \vec{y} - \frac{1}{n} \tilde{\vec{x}}_j^\T \ones \vec{y}^\T \ones\right)}{s\left(\tilde{\vec{x}}_j^\T \tilde{\vec{x}}_j - \frac{1}{n}(\tilde{\vec{x}}_j^\T \ones)^2 + (1 - \alpha) \lambda\right)}\label{eq:orthogonal-solution}                                                                                                  \\
              & = \st_{\frac{\alpha\lambda}{s\left(\tilde{\vec{x}}_j^\T \tilde{\vec{x}}_j - \frac{1}{n}(\tilde{\vec{x}}_j^\T \ones)^2 + (1 - \alpha)\lambda\right)}}\left(\frac{\tilde{\vec{x}}_j^\T \vec{y} - \frac{1}{n} \tilde{\vec{x}}_j^\T \ones \vec{y}^\T \ones}{s\left(\tilde{\vec{x}}_j^\T \tilde{\vec{x}}_j - \frac{1}{n}(\tilde{\vec{x}}_j^\T \ones)^2 + (1 - \alpha)\lambda\right)}\right), \\
...
Now, assume that \(\mat{X}\) and \(\vec{\beta}\) are fixed and that \(\vec{y} = \mat{X}\vec{\beta} + \vec{\varepsilon}\) with \(\varepsilon_i\) being identically and independently distributed noise with mean zero and finite variance \(\sigma_\varepsilon^2\). We are interested in the expected value of \eqref{eq:orthogonal-solution}. We start by focusing on the numerator since the denominator is constant. Let \(Z = \tilde{\vec{x}}_j^\T \vec{y} - \frac{1}{n} \tilde{\vec{x}}_j^\T \ones \vec{y}^\T \ones\). We have
\[
  \begin{aligned}
...
\[
  \begin{aligned}
    \var Z = \sigma & = \var\left(\tilde{\vec{x}}_j ^\T \vec{\varepsilon}\right) + \left(\frac{1}{n}\tilde{\vec{x}}_j^\T \ones\right)^2 n \var(\varepsilon) \\
                    & = \frac{\sigma_\varepsilon^2}{n}\big( \lVert \tilde{\vec{x}}_j\rVert_2^2 +(\tilde{\vec{x}}_j^\T \ones)^2\big).
  \end{aligned}
\]
...
The mean-squared error of soft-thresholding of a random variable \(Z\) with mean \(\mu\) is
\[
  \E\big( \st_\lambda(Z) - \mu\big)^2  = \int_{-\infty}^{-\lambda}(z + \lambda - \mu)^2 f_Z(z)\du z + \int_{\lambda}^{\infty}(z - \lambda - \mu)^2 f_Z(z)\du z + \mu^2 \int_{-\lambda}^\lambda f_Z(z) \du z.
\]

\subsubsection{Mean-Centering and Normally Distributed Noise}
...
\[
  Z \sim \normal\left(\tilde{\vec{x}}_j^\T\vec{x}_j \beta_j, \frac{\sigma_\varepsilon^2}{n} \lVert \tilde{\vec{x}}_j\rVert_2^2 \right).
\]
Starting with the bias, we have
\begin{align}
  \label{eq:mean-centering-bias}
  \E \st_\lambda(Z) - \mu & = \int_{-\infty}^\frac{-\lambda -\mu}{\sigma} (\sigma u + \lambda + \mu) \phi(u) \du u + \int_\frac{\lambda - \mu}{\sigma}^\infty (\sigma u - \lambda + \mu) \phi(u) \du u - \mu \\
                          & = (-\lambda - \mu) \Phi\left(\frac{-\lambda-\mu}{\sigma}\right) + \sigma \phi\left(\frac{-\lambda-\mu}{\sigma}\right)                                                            \\
                          & \phantom{=} + (\mu - \lambda) \Phi\left(\frac{\mu - \lambda}{\sigma}\right) + \sigma \phi\left(\frac{\mu-\lambda}{\sigma}\right) - \mu.
\end{align}
where \(\phi(u)\) and \(\Phi(u)\) are the probability and cumulative density functions of the standard normal distribution.

Next, for the mean-squared error, let \(\risk(\lambda, \mu) = \E \big(\st_\lambda(Z) - \mu\big)^2\) for brevity. And observe that
\[
  \risk(\lambda, \mu) = \int_{-\infty}^\frac{-\lambda - \mu}{\sigma} (\sigma u + \lambda)^2 \phi(u)\du u +
  \int_{\frac{\lambda - \mu}{\sigma}}^\infty (\sigma u - \lambda)^2 \phi(u)\du u + \mu^2 \int_{\frac{-\lambda - \mu}{\sigma}}^{\frac{\lambda - \mu}{\sigma}} \phi(u)\du u,
\]
Expanding, we have
\[
  \begin{aligned}
    R(\lambda,\mu) & = \sigma^2 \int_{-\infty}^\frac{-\lambda - \mu}{\sigma} u^2 \phi(u)\du u + \sigma^2 \int_\frac{\lambda - \mu}{\sigma}^\infty u^2 \phi(u)\du u + 2\lambda \int_{-\infty}^\frac{-\lambda - \mu}{\sigma} u \phi(u)\du u \\
                   & \phantom{=} - 2\lambda \int_\frac{\lambda - \mu}{\sigma}^{\infty} u \phi(u)\du u + \lambda^2 \int_{-\infty}^\frac{-\lambda-\mu}{\sigma} \phi(u)\du u + \lambda^2 \int_\frac{\lambda-\mu}{\sigma}^\infty \phi(u)\du u \\
                   & \phantom{=} + \mu^2 \int_\frac{-\lambda - \mu}{\sigma}^\frac{\lambda - \mu}{\sigma} \phi(u) \du u.
  \end{aligned}
\]
...
Now assume that \(\vec{x}\) is a binary feature with class balance \(q\). Then, abusing notation, we let
\[
  \mu(q) = \frac{nq(1 - q)}{s}, \qquad \sigma^2(q) = \frac{\sigma_\varepsilon^2q(1 - q)}{s^2}.
\]

% \begin{theorem}
%   If \(\vec{x}\) is a binary feature with class balance \(q\), then \(\E \st_\lambda(Z) - \mu\) is constant with respect to \(q\) if and only if \(s \propto q(1 - q)\).
% \end{theorem}
% \begin{proof}
%   Assume that \(s = f(q)q(1 - q)\). Then
%   \[
%     \lim_{q \rightarrow 1} \mu = \frac{n\beta_j}{\lim_{q \rightarrow } f(q)},\quad\lim_{q \rightarrow 1} \sigma^2 = \frac{\sigma_\varepsilon^2 n}{\lim_{q \rightarrow 1} f(q)q(1-q)}.
%   \]
% \end{proof}
