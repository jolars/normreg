...
\begin{theorem}
  ...
  Let \(X_1, X_2, \dots, X_n\) be a sample of normally distributed random variables, each with mean \(\mu\) and standard deviation \(\sigma\). Then \textcolor{red}{Define $\approx$ if you are going to prove anything.}
  \[
...
\[
  ...
  \end{cases}
\]
The solution to this system is well-known~\parencite{donoho1994}: \textcolor{red}{$s_j?$ Also maybe mention what it is, also $d_j$}
\begin{align}
...
\end{align}
with \textcolor{red}{Is this $d$ really correct?}
\[
...
\begin{equation}
  \label{eq:bias}
  \E \left( \hat\beta_j - \beta_j^* \right) = \frac{\E \st_\lambda(Z)}{d} - \beta^*_j.
\end{equation}
