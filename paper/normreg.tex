...
In the sequel, we assume that our feature is centered, that is, \(c = \sum_{i=1}^n x_i / n\). This means that the locations of the features have no impact on the coefficients in the normalized space. In addition, we let our feature be sampled from a random variable \(X\), and let \(\tilde X\) be the normalized version of this random variable.
...
\begin{proposition}
  The first predictor to enter the elastic net path if given by
  \[
    \argmin_j\left| \tilde{\vec{x}}_j^\T\left(\vec{y} - \frac{1}{n}\sum_i y_i \right)\right|.
  \]
\end{proposition}
\begin{proof}
  See x for a proof.
...
Assuming that \(\beta_1\) is the coefficient for a normally distributed feature with mean 0 and standard deviation 0.5, taking the expectation and solving for \(\beta_2\), we find
\[
  \beta_2 = \frac{1}{2 \sqrt{q(1 - q)}}.
\]
This, for instance means that if \(q = 0.1\), the coefficient for the binary feature needs to be 5/3 times larger than the continuous one to enter the model at the same time. If \(q=0.01\), the respective figure is roughly five.
