...
Now, assume that \(\mat{X}\) and \(\vec{\beta}\) are fixed and that \(\vec{y} = \mat{X}\vec{\beta} + \vec{\varepsilon}\) with \(\varepsilon_i\) being identically and independently distributed noise with mean zero and finite variance \(\sigma^2\). We are interested in the expected value of \eqref{eq:orthogonal-solution}. We start by focusing on the numerator since the denominator is constant. Let \(Z = \tilde{\vec{x}}_j^\T \vec{y} - \frac{1}{n} \tilde{\vec{x}}_j^\T \ones \vec{y}^\T \ones\) with mean \(\mu\). We have
\[
  \begin{aligned}
    \mu = \E Z & = \E \left( \tilde{\vec{x}}_j^\T (\vec{x}_j\beta_j + \vec{\varepsilon}) - \frac{1}{n} \tilde{\vec{x}}_j^\T \ones (\tilde{\vec{x}}_j\beta_j + \vec{\varepsilon})^\T \ones \right) \\
...
\[
  \begin{aligned}
    \var Z & = \var\left(\tilde{\vec{x}}_j ^\T \vec{\varepsilon}\right) + \left(\frac{1}{n}\tilde{\vec{x}}_j^\T \ones\right)^2 n \var(\varepsilon) \\
           & = \sigma^2 \lVert \tilde{\vec{x}}_j\rVert_2^2 +\frac{1}{n}(\tilde{\vec{x}}_j^\T \ones)^2 \sigma^2.
  \end{aligned}
\]
...
Next, assume that \(\tilde{\vec{x}}_j\) is mean-centered, such that \(\tilde{\vec{x}}^\T \ones = 0\), and that \(\vec{\varepsilon}\) is normally distributed. Then
\[
  Z \sim \normal\left(\tilde{\vec{x}}_j^\T\vec{x}_j \beta_j, \sigma^2\lVert \tilde{\vec{x}}_j\rVert_2^2 \right).
\]
Let \(\risk(\lambda, \mu) = \E \big(\st_\lambda(Z) - \mu\big)^2\) for brevity. And observe that
\[
...
\]
where \(\phi(u)\) is the probability density function of the standard normal distribution. Expanding the integrals, we have
\[
  \begin{aligned}
...
% TODO: Not strictly true since we are scaling here.
We now turn to the problem of computing the mean squared error across multiple features corresponding to signals and noise. We have
\[
  \E \big(\hat\beta - \beta) = \sum_{j = 1}^p \risk(\lambda, \mu) \leq p \risk(\lambda, 0) + \sum_{j=1}^p \min(\sigma^2 + \lambda^2, \mu_j^2)
\]
