...
In the sequel, we assume that \(c = \sum_{i=1}^n x_i / n\), which means that the locations of the features have no impact on the coefficients in the normalized space.

Now, we start with the simplest case, where all entries in \(\mat{X} = \vec{x}\) are independently sampled from a normally distributed random variable \(X\sim \operatorname{Normal}(\mu,\sigma)\). Furthermore, we let \(\mu = 0\) and assume that we are using standardization, that is, mean and standad deviation normalization. In this case, we have
\[
  \E \hat\beta = \frac{\softthreshold{\left( \frac{n \beta}{\sigma} ; \alpha\lambda\right)}}{\sigma^2 \left( n + (1-\alpha)\lambda\right)}
\]
