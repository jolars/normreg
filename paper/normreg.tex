...
Starting with the first term on the left-hand side of \eqref{eq:st-variance}, we have
\begin{equation}
...
\begin{equation}
  ...
  \end{multlined}
\end{equation}
Plugging \Cref{eq:mean-centered-eval,eq:mc-var-part1,eq:mc-var-part2} into \eqref{eq:betahat-variance} yields the variance of the estimator. Consequently, we can also compute the mean-squared error via the bias-variance decomposition
\begin{equation}
...
We are mainly interested in examining the case when \(\vec{x}_j\) are imbalanced. In \Cref{thm:binary-varscale-bias}, we show what the
...
\Cref{thm:binary-varscale-bias} shows that the bias of the lasso estimator when \(0 \leq \delta < 1/2\), which includes to the case of min--max and max--abs normalization (no scaling), approaches
