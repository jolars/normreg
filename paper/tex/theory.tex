...
Then the bias and variance of \(\hat{\beta}_j\) are given by
\(\E \hat\beta_j - \beta_j^* = d_j^{-1}\E \st_\lambda({Z_j}) - \beta^*_j\) and
\(\var \hat\beta_j = d_j^{-2} \var \st_\lambda({Z_j})\),
% \begin{align*}
...
\begin{align}
  \E \st_{\lambda_1}({Z_j})
  % & = \int_{-\infty}^\frac{\theta_j}{\sigma_j} (\sigma_j u - \theta_j) \pdf(u) \du u                                                                     \nonumber  \\
...
\[
  {Z_j} = \frac{\beta_j^* n(q_j - q_j^2) - \vec{x}_j^\T \vec{\varepsilon}}{s_j}, \quad
  d_j  = \frac{n(q_j - q_j^2)}{s_j} + s_j\lambda_2,
\]
% \[
%   \begin{aligned}
...
which is to use the weighted elastic net, in which we minimize the following objective:
\[
  \begin{multlined}
