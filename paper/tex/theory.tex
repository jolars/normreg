...
\Cref{eq:noiseless-estimator}, however, suggests a natural alternative to scaling of the design matrix, which is to introduce feature-wise scaling in the penalty term. In \Cref{eq:noiseless-estimator}, this would entail setting \(s_j=1\) and multiplying \(\lambda_1\) and \(\lambda_2\) with \(s_j\). Formally, we transform the objective function \(f\) in \Cref{eq:elastic-net} into
\[
