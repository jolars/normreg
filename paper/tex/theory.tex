\section{Bias and Variance of the Elastic Net}%
\label{sec:theory}
...
however, we will assume that \(\vec{\varepsilon}\) is normally distributed, under which
both the bias and variance of \(\hat{\beta}_j\) have analytical
expressions~(\Cref{sec:normally-distributed-noise}).
...
interactions~(\Cref{sec:interactions}), the choice in fact matters.
...
the bias in this case. In \Cref{sec:binary-weighting} we will show how to tackle this issue
for the elastic net by scaling the penalty weights. But for now we continue to study the
...
balanced (\(q_j = 1/2\)), normalization. Using \(\delta = 0\), for instance, scales
\(\beta_j^*\) in the input to \(\st_\lambda\) by \(n (q_j - q_j^2) = n/4\). \(\delta = 1\),
in contrast, imposes no such scaling in the class-balanced case. And for \(\delta = 1/2\),
the scaling factor is \(n/2\). Generalizing this, we see that to achieve equivalent scaling
in the class-balanced case for all types of normalization, under our parameterization, we
would need to use \(s_j = 4^{\delta - 1} (q_j - q_j^2)^\delta\). But this only resolves the
issue for the lasso. To achieve a similar effect for ridge regression, we would need
another (but similar) modification. Since all features are binary under our current
assumptions we will for now just scale \(\lambda_1\) and \(\lambda_2\) to account for this
effect,\footnote{We use this strategy in all of the following examples.} which is
equivalent to modifying \(s_j\). We will return to this issue in \Cref{sec:mixed-data},
when we consider mixes of binary and normal features, in which case this has significant
implications.
...
\vec{y}\) due to the scaling factor that scales the measurement noise upwards. Then, as
\(q_j\) approaches 1, the probability picks up again and eventually approaches 1 for
\(\delta \in \{1, 1.5\}\). The reason for this is that the variance of
\(\tilde{\vec{x}}_j^\T \vec{y}\) eventually explodes, removing the soft-thresholding effect
...
\Cref{thm:classbalance-bias,thm:classbalance-variance,cor:ridge-variance} is that there is
...
hyper-optimization, which we consider in \Cref{sec:experiments-hyperparameter}. In
...
\Cref{sec:power-fdr-multiple} we present results on power, false discovery rates, and for
multiple features.
...
that, and that, without loss of generality \(\bar{x}_1 = 0\), then we need the following
equality to hold:
%
\begin{align}
...
\citet{gelman2008}, ut want to stress that the choice of \(\kappa\) should, if possible, be
based on contextual knowledge of the data and that our results depend only superficially on
...
then given by \(x_{3,i} = (x_{1,i} - \bar{\bm{x}}_1)(x_{2,i} - \bar{\bm{x}}_2)\). See
\Cref{sec:centering-interactions} for motivation for why centering is important.
...
(q_j - q_j^2)^\omega\). On the other hand, the asymptotic results differ slightly as we now
show.
...
\Cref{fig:binary-onedim-bias-var-elnet}~(\Cref{sec:additional-theory-weighted-elnet}).
