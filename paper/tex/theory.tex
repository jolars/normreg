...
observe that since \(c_j = \bar{\bm{x}}_j\),
\[
  \begin{aligned}
...
The elastic net can be extended to include interaction terms between the features. Previous
literature on this topic has focused on hierarchical
interactions~\citep{bien2013,zemlianskaia2022,lim2015} but have not dealt with the possible
influence of normalization. Here, we will consider simple interactions with no restriction
on the presence of main effects. We also limit our discussion to pairwise interactions. For
our simplified analysis, we let \(\vec{x}_1\) and \(\vec{x}_2\) be two features of the data
and \(\bm{x}_3\) their interaction. Our model is \(\bm{y}=\beta_0 + \bm{X\beta} +
\bm{\varepsilon}\) as before, with \(\beta_3\) representing the effect of the interaction.
...
centered), which would mean that the encoding of values of the binary feature would
directly affect the interaction term.
...
bias--variance tradeoff with respect to \(\omega\).
