...
\]
where \(\sigma_2\) is the standard deviation of \(\vec{x}_2\) and \(\kappa > 0\) is the comparability scaling factor, which represents the number of standard deviations (of the continuous feature) we consider achieves comparability between the features' effects.
...
Starting with the lasso case, \(\lambda_2 = 0\), we observe that \(s_1 = \kappa (q - q^2)\) suffices for equality.
If we have the ridge case, \(\lambda_1 = 0\), on the other hand, then \(s_1 = (q - q^2)^{1/2}\) will have the same effect.
