...
(columns) \(\vec x_j\), \(\varepsilon_i\) identically and independently distributed noise,
and where we assume \(\mat{X}\), \(\beta_0^*\), and \(\vec{\beta}^*\) to be fixed. We will
also assume that the features of the normalized design matrix are orthogonal, that is,
...
case, the solution to the coefficients in the elastic net problem is given by
%
\begin{equation}
