...
\Cref{eq:orthogonal-solution-normalized}. Let \({Z_j} = \tilde{\vec{x}}_j^\T \vec{y}\) and
\[
...
\end{equation}
where \(\nu_j\) is the uncorrected sample variance of \(\vec{x}_j\). See
...
Now we also add the additional assumption that \(\vec{\varepsilon}\) is normally
distributed, under which the bias and variance of \(\hat{\beta}_j\) has an analytical
