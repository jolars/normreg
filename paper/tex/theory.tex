...
\]
%
We obtain the bias and variance of the elastic net estimate with respect to \(q_j\) by
inserting \(\tilde{\vec{x}}_j^\T \vec{y}\) and \(d_j\) into \Cref{eq:bias,eq:variance}.
...
modification. When all features are binary, we can jsut scale \(\lambda_1\) and
...
of binary and normal features in \Cref{sec:mixed-data}, we need to be more careful.
...
center\footnote{See \Cref{sec:centering-interactions} for motivation for why centering is
