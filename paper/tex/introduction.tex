...
for two data sets and two types of normalization. The choice of normalization has a strong
effect, yielding completely diffent paths in terms of both feature selection and size of
the coefficients.
...
\begin{figure}[bpt]
  ...
  \caption{%
    ...
    \data{w1a}~\citep{platt1998} using two types of normalization:
    standardization and maximum absolute value normalization (max--abs).
    (See \Cref{sec:data-summary} for more information about these
    data sets.) For each dataset, we have colored the coefficients if they were
    among the first five
    to become non-zero under either normalization scheme.
  }
...
computational aspects such as optimization performance and data storage or by being
``standard''.
% At the time of writing, for instance, the popular machine learning library
% \texttt{scikit-learn}~\citep{scikit-learndevelopers2024} recommends maximum absolute value
% normalization in the particular case of sparse data. Yet, as we have already seen
% (\Cref{fig:realdata-paths}), this may have dramatic effects for the results.

Standardization is a natural choice when the features are normally distributed. But for
other types of data the choice is not as straightforward. In particular, there is no clear
approach to normalizing binary features (where the each observations takes either of two
values). Anecdotal suggestions include not normalizing at all or to normalize as you would
if it were continuous data---the implications of either of these suggestions (or in deed
any other), however, have yet to be investigated.
...
deviations), then scaling must be modified to take this into account. Finally, we briefly
look at a the case of interactions between normal and binary features and provide
suggestions for how to normalize the features in this case.
