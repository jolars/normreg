...
generated two binary features with class balances 0.5 and 0.9, respectively, and fit a full
regularization path using lasso and ridge regression. The true coefficients are set to one
...
depending on whether L1 or Max--Abs normalization is used.

\begin{figure}[htpb]
  \centering
  \includegraphics[]{small_path_experiment.pdf}
  \caption{%
    Unstandardized coefficients for lasso and ridge regression along
    a regularization path. The models are fit to an
    experiment with two binary features, generated to have class balances 0.5
    and 0.9, respectively. We have generated the response as $y_i =
      \boldsymbol{x}_i^\intercal \boldsymbol{\beta}$, with $\boldsymbol{\beta} =
