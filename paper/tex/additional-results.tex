...
representations with respect to the other features given that one-hot encoding results in a
set of mutually exclusive features. This means that the same issues with variance and
...
standard approach is to one-hot encode it into a set of binary variables $X_1 =
  \boldsymbol{1}\{Z=1\}$, $X_2 = \boldsymbol{1}\{Z=2\}$, and $X_3 = \boldsymbol{I}\{Z=3\}$,
...
same comparison. Their class balances, however, of the features are very different.
...
The plot shows that $\beta_{C:A}$ enters the path earlier than $\beta_{A:C}$, despite
converging to the same value as $\lambda \to 0$. This means that the choice of reference
...
\begin{figure}[htpb]
  ...
  \caption{%
    The coefficients along the regularization paths
  }
