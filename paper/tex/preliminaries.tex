...
most common choices in \Cref{tab:normalization-types}.
...
statistics. One of its benefits is that it simplifies certain aspects of fitting the model.
For instance, the intercept term \(\hat\beta_0\) is equal to the mean of the response
\(\vec y\). For regularized methods, it is typically the case that we standardize with the
uncorrected sample standard deviation (division by \(n\)). The downside of standardization
is that it involves centering by the mean, which typically destroys sparsity in the data
structure. This is not a problem when the data is stored as a dense matrix; but when the
data is sparse, it can lead to a significant increases in memory usage and processing time.

A common alternative to standardization, particularly when data is sparse, is to scale the
...
\Cref{thm:maxabs-gev}~(\Cref{sec:additional-theory}), we study how this effect comes into
...
\emph{location} of the data, only its \emph{spread}. In norm-scaling, the norm can be any
...
where it is the Euclidean norm. It can of course be used together with centering by some
location, and in the case when \(p=2\) this would amount to standardization. A special case
of normalization is the adaptive lasso~\citep{zou2006}, which is a two-step procedure. In
the first step a model, often ordinary least-squares regression (OLS) or ridge regression,
is fit to the data. The estimated coefficients from the model are then used to scale the
features.

\subsection{The Lasso, Ridge Regression, and the Elastic Net}

The elastic net is represented by the following convex optimization problem:
\begin{equation}
...
When \(\lambda_1 > 0\) and \(\lambda_2 = 0\), the elastic net is equivalent to the lasso, and when \(\lambda_1 = 0\) and \(\lambda_2 > 0\), it is equivalent to ridge regression. Expanding \(f\) in \Cref{eq:elastic-net}, we have
\[
  \begin{aligned}
...
\subsection{Weighted Penalization}\label{sec:weighted-elasticnet}
...
\Cref{eq:elastic-net} given the normalized design \(\tilde{\mat{X}}\) minimize the
following objective:
\[
...
coefficients, which will now be on the scale of the normalized features. But we are
...
net~(\Cref{sec:weighted-elasticnet}), since the estimated coefficients are then on the same
