...
In this paper we focus on the lasso and ridge regression as well as their generalization:
...
\end{equation}
which can be seen by substituting \(\mat{S}\vec{\beta} = \tilde{\vec{\beta}}\) in \Cref{eq:elastic-net}. Note that this one-to-one correspondence is specific to the elastic net. The connection is not guaranteed to hold for other choices of \(g\), and even if it does might require different choices of \(\vec{u}\) and \(\vec{u}\).
