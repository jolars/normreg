...
section into two parts, focusing in the first part (\Cref{sec:experiments-lassoridge}) on
...
\(\vec{x}_j\) uniformly at random. In each case, we fit either the lasso (the elastic net
with \(\lambda_1 = \alpha \lambda\) ) or ridge (the elastic net with \(\lambda_2 = (1 -
\alpha)\lambda\)).
