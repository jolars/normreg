...
\begin{figure}[htpb]
  ...
  \caption{%
    ...
    for combinations of \(\delta\) and \(\lambda\) in ridge regression. The
    dotted path shows the smallest NMSE as a function of \(\lambda\). The dot marks the
    combination with the lowest error.
  }
...
\emph{Strategy 1}, we simply standardize the resulting interaction feature.
...
\begin{figure}[htpb]
  ...
  \caption{%
    ...
    an interaction feature. We use two different normalization strategies where
    Strategy 1 represents standardization and Strategy 2 is mean-centering
    togeher with scaling by \(s_1 s_2\). Here we have set \(\beta^*_j = 1\) for \(j \in [3]\),
    but see \Cref{fig:interactions-full}~(\Cref{sec:additional-experiments-interactions})
    for results on different combinations of true effects.
  }
