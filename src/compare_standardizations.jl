...
function compare_standardizations(x, y; dist = Normal())
  x = Array(x)

  x_std, x_std_center, x_std_scale = normalize(x, "mean_std")
  x_max, x_max_center, x_max_scale = normalize(x, "max_abs")

  res_std = fit(LassoPath, x_std, y, dist, standardize = false)
  res_max = fit(LassoPath, x_max, y, dist, standardize = false)

  intercepts_std, coefs_std =
    unstandardize_coefficients(res_std.b0, res_std.coefs, x_std_center, x_std_scale)
  intercepts_max, coefs_max =
    unstandardize_coefficients(res_max.b0, res_max.coefs, x_max_center, x_max_scale)
